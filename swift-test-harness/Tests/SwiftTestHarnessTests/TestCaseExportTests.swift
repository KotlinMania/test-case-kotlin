...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TestCase.swiftmodule/`
...
//      `import TestCase` above would fail at compile time.
//
//   2. The static archive `libTestCase.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TestCase" }` and
//      `flattenPackage = "io.github.kotlinmania.testcase"` configuration in
...
//      via the `TestCaseLibrary` product.
//
...
final class TestCaseExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TestCase swift module imported cleanly")
    }
}
