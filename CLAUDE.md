# Claude Code Project Instructions — test-case-kotlin

## Project Overview

This is **test-case-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`test-case`](https://crates.io/crates/test-case).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/test-case/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/test-case>
